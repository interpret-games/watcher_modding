...
const BOSSSPEED = 175; // How fast the boss moves
const BOSSPROJSPEED = 225; // How fast the boss projectiles move
...
async function onBeforeProjectStart(runtime) {
  ...
  tex = {
    ...
    bIdle: runtime.objects.TexBossIdle,
    bAttack: runtime.objects.TexBossAttack,
...
function checkBossTimer(runtime) {
  ...
  if (tm.hasFinished("bossAttackTimer")) {
    ...
    if (newAttack < 1) {
      ...
      tm.startTimer(0.5, "bossFloorBlastTimer", "once"); // Start floor blast sequence

      // Projectile attack
    } else if (newAttack == 2) {
      tex.bAttack.getFirstInstance().startAnimation("beginning");
      bossModel.setFaceObject("right", tex.bAttack);
      tm.startTimer(0.5, "bossProjectilesTimer", "once"); // Start projectile attack sequence

      // Projectile attack
    // } else if (newAttack == 3) {
    //   tex.bAttack.getFirstInstance().startAnimation("beginning");
    //   bossModel.setFaceObject("right", tex.bAttack);
    //   tm.startTimer(0.5, "`bossFieldProjectilesTimer`", "once"); // Start projectile attack sequence
  ...
  function bossRandomMove() {
    ...
    bossCol.behaviors.Tween.startTween(
      "position",
      [newPosX, newPosY],
      dist2D(bossCol.x, bossCol.y, newPosX, newPosY) / BOSSSPEED,
      "in-out-sine"
    );
    tm.startTimer(
      dist2D(bossCol.x, bossCol.y, newPosX, newPosY) / BOSSSPEED,
  ...
  if (tm.hasFinished("bossFloorBlastExplosionTimer")) {
    ...
    if (dist2D(playerCol.x, playerCol.y, bossFB.x, bossFB.y) < bossFB.width / 2)
      playerDeath(runtime);
  ...
  if (tm.hasFinished("bossProjectilesExplodeTimer")) {
    ...
    tex.bFBExp.getFirstInstance().startAnimation("beginning");
    for (const p of runtime.objects.BossProjectile2.getAllInstances()) {
...
function getInputs(runtime) {
  ...
  if (keyboard.isKeyDown("Enter") && !inputsEnabled) {
    ...
    if (playerModel.height == 0 || bossCol.zElevation == -64) {
      ...
      uiLifeBarSupport.behaviors.Tween.startTween(
        "opacity",
        1,
        0.5,
        "in-out-sine"
  ...
  // Perform Dash
  if (
    keyboard.isKeyDown("ControlLeft") &&
    !pv.dashInCooldown &&
    pv.dashing == "N" &&
    !pv.isAttacking
  ) {
  ...
  if (
    ...
    if (dist2D(playerCol.x, playerCol.y, bossCol.x, bossCol.y) < 76) {
      // Boss flashes
...
function playerDashState(runtime) {
  ...
  if (pv.dashing == "N" && !pv.isWalking && !pv.isAttacking)
    playerModel.setFaceObject("left", tex.pIdle);
}
...
function rotateBillboards(runtime) {
  ...
  for (const exp of runtime.objects.BossFBExpModel.getAllInstances())
    exp.angle = Math.atan2(
      camera.getCameraPosition()[1] - exp.y,
      camera.getCameraPosition()[0] - exp.x
    );

  // Projectiles always look at the camera
  for (const p of runtime.objects.BossProjectile.getAllInstances())
    p.angle = Math.atan2(
      camera.getCameraPosition()[1] - p.y,
      camera.getCameraPosition()[0] - p.x
    );
}
